...
An AI workspace for analysing complex document collections — research, investigations, reports, corporate records, archives, compliance files. The non-negotiable product invariant: **every AI interaction is matter-scoped, grounded in uploaded documents, citation-backed, and audit-logged.** Any feature that generates AI output must create an `AIInteraction` row with `Citation` rows linking claims to `DocumentChunk`s, and must never retrieve context across matter boundaries.

**Positioning:** CaseOS is domain-neutral. Do not reintroduce legal/lawyer/court framing in UI copy, prompts, or docs. The `Matter` and `Firm` models are internal names only — users see "project" and "workspace". The `ATTORNEY`/`PARALEGAL` Role enum values are legacy database identifiers mapped to neutral labels in `lib/format.ts`.
...
npm run db:reset-embeddings  # re-embed every document (required after changing EMBEDDING_MODEL/DIMENSIONS)
...
- **Ingestion pipeline** — `lib/ingest/`: `extract.ts` (unpdf, page-aware) → `chunk.ts` (~2000 chars, 200 overlap, never spans pages) → `embed.ts` → `pipeline.ts` orchestrates and writes status transitions + audit rows. Upload is two-step in `lib/actions/documents.ts`: `prepareDocumentUpload` returns a Supabase signed URL the browser PUTs to directly, then `finalizeDocumentUpload` ingests by storage path. The file never crosses the serverless function — Vercel caps request bodies at 4.5 MB.
- **Embeddings go through the Gemini API** — `gemini-embedding-001` at 768 dims (`EMBEDDING_MODEL` / `EMBEDDING_DIMENSIONS`), reusing `GEMINI_API_KEY`. Uses `taskType: RETRIEVAL_DOCUMENT` for chunks and `RETRIEVAL_QUERY` for queries; these are asymmetric and must stay paired. Sub-3072 vectors come back truncated and un-normalized, so `embed.ts` normalizes before storing — pgvector cosine assumes unit length. xAI/Grok has **no embeddings endpoint** — do not try to use it for embeddings. Chunk and query vectors must come from the same model *and width*; changing either requires `npm run db:reset-embeddings` (which deletes existing `Citation` rows, since chunks are replaced).
  - This replaced a local `Xenova/bge-small-en-v1.5` embedder. Do not reinstate it on serverless: `@huggingface/transformers` + `onnxruntime-node` are ~349 MB against Vercel's 250 MB function limit, the ONNX binary is platform-native, and transformers.js needs a writable model cache that Vercel does not provide outside `/tmp`.
...
- **AI pipeline** — `lib/ai/`: `retrieve.ts` (matter boundary enforced inside the SQL join, top-8 cosine via `<=>`), `generate.ts` (sources-only system prompt; `parseCitations` maps `[Sn]` markers to sentences). Generation goes through a **provider abstraction**: `provider.ts` exposes `getChatProvider()`, selected by `AI_PROVIDER` (`gemini` default | `grok` | `openai`); each provider in `lib/ai/providers/` implements the `ChatProvider` interface (`generate({system, user, temperature}) → {text, model}`). Gemini uses the native `@google/genai` SDK; Grok and OpenAI share `openai-compatible.ts` (OpenAI SDK, different baseURL/key/model). Switching providers never touches retrieval, prompting, citation parsing, or persistence. The `askQuestion` action in `lib/actions/ai.ts` persists `AIInteraction` + `Citation` rows + audit entry in one transaction; `reviewInteraction` is the human approve/reject gate. Test with `npm run test:ai` (skips generation with exit 2 if the active provider has no credits/quota).
